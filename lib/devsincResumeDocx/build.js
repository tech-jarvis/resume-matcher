...
function skillLine(label, value) {
  ...
  return para(
    [
      run(`${label}: `, { bold: true, color: COLORS.heading }),
      run(value),
    ],
    { after: 60 }
  );
...
function buildMainColumn(resume) {
  const blocks = [
    para([run(resume.name, { bold: true, size: SIZE.name, color: COLORS.heading })], {
      after: 40,
    }),
  ];
  ...
  if (resume.designation) {
    blocks.push(
      para([run(resume.designation, { size: SIZE.designation, color: COLORS.heading })], {
        after: 80,
      })
    );
  }
    ...
    for (const p of resume.projects) {
      blocks.push(
        ...projectBlock({
          name: p.name,
          link: p.link,
          linkLabel: p.link ? p.linkLabel || p.link : undefined,
...
function buildSidebarColumn(resume) {
  const blocks = [...sectionTitle("Contact")];

  const email = resume.contact?.email?.trim();
  const salesEmail = resume.contact?.salesEmail?.trim() || DEVSINC_SALES_EMAIL;
  // No placeholders: if the candidate's own email isn't known, fall back to the sales email.
  if (email) {
    blocks.push(para(email, { after: 60 }));
  }
  blocks.push(para(salesEmail, { after: 140 }));
  ...
  if (resume.achievements?.length > 0) {
    ...
    for (const a of resume.achievements) {
      blocks.push(
        para([run(a.title, { bold: true, color: COLORS.heading })], { before: 60, after: 30 })
      );
      if (a.description) {
  ...
  if (resume.education?.length > 0) {
    ...
    for (const e of resume.education) {
      if (e.institution) {
        blocks.push(
          para([run(e.institution, { bold: true, color: COLORS.heading })], {
            before: 60,
            after: 30,
          })
        );
