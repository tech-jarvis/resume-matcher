...
export function sectionTitle(title) {
  return [
    para([run(title.toUpperCase(), { bold: true, size: SIZE.section, color: COLORS.heading })], {
      before: 200,
      after: 40,
    }),
...
export function jobTitleRow(title, dates) {
  return new Paragraph({
    ...
    children: [
      run(title, { bold: true, size: SIZE.body, color: COLORS.heading }),
...
export function projectBlock(project) {
  ...
  if (project.link) {
    ...
    blocks.push(
      para([run(project.name, { bold: true, color: COLORS.heading })], {
        before: 100,
        after: 40,
      })
    );
